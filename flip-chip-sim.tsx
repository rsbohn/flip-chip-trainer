    ...
    <div className="flex flex-col h-screen bg-black text-gray-100 font-mono">
      ...
      <div className="bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 border-b-4 border-black p-6 shadow-2xl">
        <div className="px-4">
          ...
          <div className="text-2xl font-bold tracking-wider text-gray-300" style={{fontFamily: 'serif', textShadow: '0 1px 2px rgba(0,0,0,0.8)'}}>
            Flip Chip Trainer
          </div>
        </div>
      </div>
      ...
      <div className="w-80 bg-gradient-to-b from-gray-900 to-black border-r-4 border-black p-6 flex flex-col shadow-inner">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex items-center gap-1 px-3 py-2 rounded font-bold text-xs tracking-wide shadow-lg flex-1 ${
                powerOn ? 'bg-red-800 hover:bg-red-900 border-2 border-red-900' : 'bg-gray-700 hover:bg-gray-600 border-2 border-gray-800'
              }`}
            >
              <Power size={16} />
              {powerOn ? 'OFF' : 'ON'}
            ...
            <button
              ...
              disabled={!powerOn}
              className={`flex items-center gap-1 px-3 py-2 rounded font-bold text-xs tracking-wide shadow-lg flex-1 ${
                !powerOn ? 'bg-gray-700 border-2 border-gray-800 cursor-not-allowed opacity-50' :
                running ? 'bg-orange-800 hover:bg-orange-900 border-2 border-orange-900' : 'bg-gray-700 hover:bg-gray-600 border-2 border-gray-800'
              }`}
            >
        ...
        <div className="mb-8">
          ...
          <div className="space-y-2 bg-black p-3 rounded border border-gray-800">
            <div className="flex items-center gap-2">
              <div className={`w-3 h-3 rounded-full border ${powerOn ? 'bg-amber-600 border-amber-500 shadow-lg shadow-amber-600/50' : 'bg-gray-800 border-gray-700'}`} />
        ...
        <div className="mt-8 p-3 bg-black rounded border border-gray-800">
          ...
          <div className="text-xs text-gray-600 mt-1 font-mono">
            Cards: {slots.filter(s => s !== null).length} 
            ({slots.filter(s => s !== null).length * 2} bits)
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4 relative z-10">
          {slots.map((card, slotIndex) => (
            <div key={slotIndex} className="flex items-center gap-4">
              ...
              {card ? (
                <div className={`bg-gradient-to-b from-blue-950 to-blue-900 border-2 rounded p-4 w-64 transition-all shadow-lg ${
